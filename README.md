...
Typically, if you have a plot designed to look good in print, when it is included in a presentation, the text on the title/axes/legend is too small to be read by everyone in the room. `ggpres` fixes that by taking a `ggplot` plot and increasing the size of the text. Usage is `ggpres(g, title_size = 24, text_size = 18)`.
